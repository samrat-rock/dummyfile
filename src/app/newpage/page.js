...
const data = [
  ...
  function Page() {
    return (
      <div className="container mx-auto mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-[1200px]">
        {data.concat(data).map((item, index) => (
          <div key={index} className="border-2 rounded-lg shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="pt-4 pb-4 px-4 text-center font-serif flex flex-col flex-1">
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {item.tags.map((tag, i) => (
                  <span key={i} className="bg-gray-200 rounded-full px-3 py-1 text-sm">
                    #{tag}
                  </span>
                ))}
              </div>
